...
BEGIN
	IF EXISTS  (SELECT distinct e.NOMBRE 
				FROM EQUIPOS e 
				WHERE e.NOMBRE like CONCAT('%',@nomEquipo,'%') OR e.DIRECTOR_TECNICO like CONCAT('%',@nomDt,'%'))
	BEGIN
		SET  @ReturnValue = 1;
	END
	ELSE
...
EXEC @ReturnValue = SP_CONSULTAR_EQUIPO 'Instituto', 'NOM',@ReturnValue;
select @ReturnValue
...
select * from personas where id = 28 --1 a 38 
